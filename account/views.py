...
def redirect_user_by_role(user):
    ...
    if user.is_superuser or user.role == "ADMIN":
        return redirect("tpo_dashboard")  # Admins go to TPO dashboard for now
    elif user.role == "TPO":
...
def is_admin_or_tpo(user):
    return user.is_authenticated and (user.is_superuser or user.role == "TPO")

@login_required
def tpo_dashboard(request):
    if request.user.role != "TPO" and not request.user.is_superuser:
        return render(request, "accounts/unauthorized.html")  # block others
    return render(request, "accounts/tpo_dashboard.html")
...
def create_tpo(request):
    if request.method == "POST":
        ...
        return redirect("tpo_list")  # Page showing all TPOs
    return render(request, "accounts/create_tpo.html")

@login_required
@user_passes_test(lambda u: u.role == "TPO" or u.is_superuser)
def create_student_account(request):
    if request.method == "POST":
        username = request.POST["username"]
        email = request.POST["email"]
        password = request.POST["password"]  # auto-generate from DOB or random
...
# ====================== OLD STUDENT VIEWS (TO BE DEPRECATED) ======================
